...
from .models import School, SchoolClass, User
...
async def login(body: schemas.LoginIn):
    ...
    return {**make_tokens(user_doc.id, user_doc.role, user_doc.school_id),
            "must_change_password": user_doc.password_temp}
...
async def refresh(body: schemas.RefreshIn):
    ...
    return {**make_tokens(user.id, user.role, user.school_id),
            "must_change_password": user.password_temp}
...
async def create_user(body: schemas.UserCreateIn, user: dict = Depends(require_role("admin"))):
    if not user.get("school_id"):
        raise HTTPException(403, "Только для админа школы")
    if body.role not in ("student", "teacher", "admin"):
        raise HTTPException(422, "Роль не в списке")
    if await service.by_login(user["school_id"], body.login):
...
def _ensure_same_school(user: dict, doc) -> None:
    """Изоляция арендатора: чужая школа неотличима от несуществующей."""
    if user.get("role") != "superadmin" and doc.school_id != user.get("school_id"):
        raise HTTPException(404, "Пользователь не найден")
...
async def create_class(body: schemas.ClassIn, user: dict = Depends(require_role("admin"))):
    if not user.get("school_id"):
        raise HTTPException(403, "Только для админа школы")
    await SchoolClass(school_id=user["school_id"], **body.model_dump()).insert()
    return {"ok": True}
...
async def list_classes(user: dict = Depends(get_current_user)):
    if not user.get("school_id"):
        return []
    rows = await SchoolClass.find(SchoolClass.school_id == user["school_id"]).to_list()
